...
from tools.pytdx.parser.ex_setup_commands import ExSetupCmd1, ExSetupCmd2
from tools.pytdx.parser.ex_get_markets import GetMarkets
from tools.pytdx.parser.ex_get_instrument_count import GetInstrumentCount
from tools.pytdx.parser.ex_get_instrument_quote import GetInstrumentQuote
from tools.pytdx.parser.ex_get_minute_time_data import GetMinuteTimeData
from tools.pytdx.parser.ex_get_history_minute_time_data import GetHistoryMinuteTimeData
from tools.pytdx.parser.ex_get_transaction_data import GetTransactionData
from tools.pytdx.parser.ex_get_history_transaction_data import GetHistoryTransactionData
from tools.pytdx.parser.ex_get_instrument_bars import GetInstrumentBars
...
class TdxExHq_API(BaseSocketClient):
    def __init__(self,is_new_version=False, multithread=False, heartbeat=False, auto_retry=False, raise_exception=False):
        super(TdxExHq_API, self).__init__(multithread=multithread, heartbeat=heartbeat, auto_retry=auto_retry, raise_exception=raise_exception)
        self.is_new_version = is_new_version

    def setup(self):
        ExSetupCmd1(self.client, lock=self.lock).call_api()
        if self.is_new_version:
            ExSetupCmd2(self.client, lock=self.lock).call_api() 
...
if __name__ == '__main__':
    ...
    with api.connect('39.108.101.114', 7727):
        # log.info("获取市场代码")
