...
import pandas as pd
from tools.pytdx.base_socket_client import BaseSocketClient, update_last_ack_time
from tools.pytdx.heartbeat import HqHeartBeatThread
from tools.pytdx.log import DEBUG, log
from tools.pytdx.params import TDXParams
from tools.pytdx.parser.get_block_info import (GetBlockInfo, GetBlockInfoMeta,
                                         get_and_parse_block_info)
from tools.pytdx.parser.get_company_info_category import GetCompanyInfoCategory
from tools.pytdx.parser.get_company_info_content import GetCompanyInfoContent
from tools.pytdx.parser.get_finance_info import GetFinanceInfo
from tools.pytdx.parser.get_history_minute_time_data import GetHistoryMinuteTimeData
from tools.pytdx.parser.get_history_transaction_data import GetHistoryTransactionData
from tools.pytdx.parser.get_index_bars import GetIndexBarsCmd
from tools.pytdx.parser.get_minute_time_data import GetMinuteTimeData
from tools.pytdx.parser.get_security_bars import GetSecurityBarsCmd
from tools.pytdx.parser.get_security_count import GetSecurityCountCmd
from tools.pytdx.parser.get_security_list import GetSecurityList
from tools.pytdx.parser.get_security_quotes import GetSecurityQuotesCmd
from tools.pytdx.parser.get_transaction_data import GetTransactionData
from tools.pytdx.parser.get_xdxr_info import GetXdXrInfo
from tools.pytdx.parser.get_report_file import GetReportFile
from tools.pytdx.parser.setup_commands import SetupCmd1, SetupCmd2, SetupCmd3, SetupCmdNew1, SetupCmdNew2, SetupCmdNew3
from tools.pytdx.util import get_real_trade_date, trade_date_sse
try:
    # Python 3
...
class TdxHq_API(BaseSocketClient):
    def __init__(self,is_new_version=False, multithread=False, heartbeat=False, auto_retry=False, raise_exception=False):
        super(TdxHq_API, self).__init__(multithread, heartbeat, auto_retry, raise_exception)
        self.is_new_version = is_new_version

    def setup(self):
        if self.is_new_version:
            SetupCmdNew1(self.client, lock=self.lock).call_api()
            SetupCmdNew2(self.client, lock=self.lock).call_api()
            SetupCmdNew3(self.client, lock=self.lock).call_api()
        else:
            SetupCmd1(self.client, lock=self.lock).call_api()
            SetupCmd2(self.client, lock=self.lock).call_api()
            SetupCmd3(self.client, lock=self.lock).call_api()
    # API List
...
if __name__ == '__main__':
    ...
    api = TdxHq_API()
    if api.connect('220.203.236.11', 7709):
        log.info("获取股票行情")
        # stocks = api.get_security_quotes([(0, "000001"), (1, "600300")])
        # pprint.pprint(stocks)
        log.info("获取k线")
        data = api.get_security_bars(4, 1, '600988', 0, 10)
        print(data)
        # log.info("获取 深市 股票数量")
