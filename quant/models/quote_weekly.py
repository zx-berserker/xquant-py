...
@dataclass
class QuoteWeekly(Base):
    __tablename__ = 'quote_weekly'
    """
    volume: 成交量（股）
    amount: 成交额（元）
    pct_chg: 涨跌幅(%)
    ...
    product = relationship('Product', back_populates='quote_weekly')
    ...
    def __repr__(self):
        return "<QuoteWeekly time:%s product_id:%s close:%f>" % (self.time, self.product_id, self.close)
