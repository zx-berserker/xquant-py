...
def update_stock_product_quote(period_type:QuotePeriodEnum=QuotePeriodEnum.DAILY, start_date:str='20060101', end_date:str="20500101", tdx_code:str=None, exchange_code=None, limit=100):
    ...
    with SQLAlchemy.session_context() as session:
        ...
        if tdx_code and exchange_code:
            is_done = True
            # str_list = symbol.split(".")
            for exg in exg_list:
                prod_list = exg.products
                if is_done:
                    if exg.code != exchange_code:
                        continue
                    for num in range(0, len(prod_list)):
                        if prod_list[num].tdx_code == tdx_code:
                            is_done = False
    ...
    for query in query_list:
        ...
        update_task_factory = CacheFileWriterTaskFactory(file_path, file_base_name, data_list, flush_count, slice_capacity, file_prefix_base_name=preflex)
        spider_factory = TdxQuoteSpiderTaskTaskFactory(tdx_period_type, start_date, end_date, limit)
