...
def find_stock_industry_id(stock_code):
    ...
    for obj in data_list:
        for key, value in obj.items():
            ...
            if code in code_list:
                industry = db.session.query(Industry).filter(Industry.code == key).first()
                return industry.id
    return None
...
def update_stock_table():
    ...
    for index in stocks_df.index:
        ...
        if int(stock_info_df.loc[0].values[4]) == 1:
            industry_id = find_stock_industry_id(code)
        stock = Stock(
            code=code,
...
def update_k_data_table(data_type='d'):
    ...
    for stock in stock_list:
        k_data_df = QueryStockInfo.query_k_data(stock.code, freq_type=data_type)
        data_list = []
        for index in k_data_df.index:
            data_dict = {
                ...
                'volume': check_df_value(k_data_df.loc[index].values[7], 0),
                'amount': check_df_value(k_data_df.loc[index].values[8], 0.0),
                'turn': check_df_value(k_data_df.loc[index].values[9],  0.0),
                'pct_chg': k_data_df.loc[index].values[10],
            }
            if data_type == 'd':
                data_dict['pre_close'] = check_df_value(k_data_df.loc[index].values[6], 0.0)
        ...
        print(k_data_df)
        db.session.bulk_insert_mappings(cls_type, data_list)


def update_stock_k_data(flask_app=None, session=None, stock=None, start_date='2006-01-01', freq_type='d'):
    if stock is None:
        print("stock is None!!!")
        return
    cls_type = KDataDaily
